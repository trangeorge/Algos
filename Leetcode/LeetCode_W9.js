...
var dominantIndex = function (nums) {
    // Make a copy of the nums array to avoid modifying the original array
    var sorted = [...nums].sort((a, b) => a - b);
    var lastNum = sorted[nums.length - 1];
    var secondNum = sorted[nums.length - 2];

    if (lastNum >= secondNum * 2) {
        return nums.indexOf(lastNum);
    } else {
        return -1;
    }
};
